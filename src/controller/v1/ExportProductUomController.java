...
import adapter.ExportProductUomAdapter;
...
@RestController
@RequestMapping("/v1")
public class ExportProductUomController {
    final static Logger logger = LogManager.getLogger(ExportProductGroupController.class);
    static Gson gson = new GsonBuilder().serializeNulls().create();
    String db_name = "";
    int port = 0;

    @RequestMapping(value = "/export-product-uom", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelProductUom(
        ...
        String url = "/export-product-uom";

        try{
            mdlProductUom paramResult1;
            paramResult1 = gson.fromJson(param, mdlProductUom.class);
...
//            db_name = "db_ffa"; //only-testing
                ...
                urlLink = ExportProductUomAdapter.ExportProductUom(paramResult1, paramResult2, port, db_name);
    ...
    @RequestMapping(value = "/export-product-uom-template", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelProductUomTemplate(
            @RequestHeader String authorization,
            @RequestHeader String key,
            @RequestHeader String timestamp,
            @RequestHeader String signature,
        ...
        String urlLink = "";
        String url = "/export-product-uom-template";
        String apiMethod = request.getMethod();
        // get all headers for logging purpose
        Map<String, String> headerList = Collections.list(request.getHeaderNames()).stream().collect(Collectors.toMap(h -> h, request::getHeader));
        String jsonHeader = gson.toJson(headerList);
        ...
        try {
            port = request.getLocalPort();
            mdlProductUom paramResult1 = gson.fromJson(param, mdlProductUom.class);
            ...
            if (_mdlResultFinal.status_code.equals("ERR-00-000")) {
                ...
                urlLink = ExportProductUomAdapter.ExportProductUomTemplate(paramResult1, paramResult2, port, db_name);
                if (!urlLink.equals("")) {
                    _mdlErrorSchema.error_code = "ERR-00-000";
                    _mdlResultFinal = ErrorSchemaAdapter.GetErrorSchema_V2(_mdlErrorSchema, port);
                    _mdlResultFinal.value = urlLink;
