...
import adapter.UserRoleAdapter;
...
public class UserRoleController {
    final static Logger logger = LogManager.getLogger(UserRoleController.class);
    static Gson gson = new GsonBuilder().serializeNulls().create();
    String db_name = "";
    int port = 0;

    @RequestMapping(value = "/get-user-role", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal GetRole(
        ...
        try {
            ...
            if (_mdlResultFinal.status_code.equals("ERR-00-000")) {
                ...
                List<mdlRole> roleList;
                roleList = UserRoleAdapter.GetRole(paramResult, db_name, port);
                int dataListTotal = UserRoleAdapter.GetTotalRole(paramResult, db_name, port);
                ;

                mdlDataTableResult dataTableResult = new mdlDataTableResult();
                dataTableResult.data = roleList;
                dataTableResult.draw = paramResult.draw + 1;
    ...
    @RequestMapping(value = "/upload-user-role", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal UploadUserRole(
            HttpServletResponse response,
            HttpServletRequest request,
            @RequestHeader String authorization,
            @RequestHeader String key,
        ...
        String url = "/upload-user-role";

        //TODO 1 (UploadUserRole) : Param untuk InsertLogApi
        String lException = "";
        String is_error = "";
        try {
            mdlRole paramResult;
            paramResult = gson.fromJson(param, mdlRole.class);
...
//            db_name = "db_ffa"; //only-testing
                db_name = ((mdlDB) _mdlResultFinal.value).db_id;
                boolean status = UserRoleAdapter.UploadRole(paramResult, db_name, port);
            ...
            _mdlResultFinal.value = ex.toString();
            logger.error(LogAdapter.logToLog4jException(startTime, url, apiMethod, systemFunction, jsonHeader, gson.toJson(param), gson.toJson(_mdlResultFinal), ex.toString()), ex);
            // TODO 3 (UploadUserRole) : Set Data exception untuk InsertLogApi
...
//            return _mdlResultFinal;
        ...
        if (is_error.equals("1") || mdlLogApiConfig.is_active == true) {
            //TODO 4 (UploadUserRole) : Set Data jsonOut dan status_api untuk InsertLogApi
            String status_api = String.valueOf(response.getStatus()) + " ; " + String.valueOf(response.getBufferSize()) + " B";
            //

            //TODO 5 (UploadUserRole) : Input data ke model.Log.mdlInsertLog modelInsertLogParam
            model.Log.mdlInsertLog modelInsertLogParam = new model.Log.mdlInsertLog();
            modelInsertLogParam.api_name = url;
            modelInsertLogParam.fuction_name = "UploadUserRole";
            ...
            //TODO 6 (UploadUserRole) : Input data yang telah diisi ke core.LogAdapter.InsertLog
...
//            db_name = "db_ffa"; //only-testing
                db_name = ((mdlDB) _mdlResultFinal.value).db_id;
                boolean status = UserRoleAdapter.DeleteRole(paramResult, db_name, port);
    ...
    @RequestMapping(value = "/get-dropdown-user-role", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal DropDownRole(
            HttpServletRequest request,
            HttpServletResponse response,
            @RequestHeader String authorization,
            @RequestHeader String key,
        ...
        mdlResultFinal _mdlResultFinal = new mdlResultFinal();
        mdlErrorSchema _mdlErrorSchema = new mdlErrorSchema();
        String url = "/get-dropdown-user-role";
...
//            db_name = "db_ffa"; //only-testing
                db_name = ((mdlDB) _mdlResultFinal.value).db_id;
                List<mdlRole> roleList;
                roleList = UserRoleAdapter.GetDropDownRole(db_name, port);

                _mdlErrorSchema.error_code = "ERR-00-000";
                _mdlResultFinal = ErrorSchemaAdapter.GetErrorSchema_V2(_mdlErrorSchema, port);
                _mdlResultFinal.value = roleList;
                response.setStatus(SC_OK);
