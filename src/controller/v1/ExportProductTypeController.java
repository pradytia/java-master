...
import adapter.ExportUomAdapter;
...
@RestController
@RequestMapping("/v1")
public class ExportProductTypeController {
    final static Logger logger = LogManager.getLogger(ExportCallPlanController.class);
    Gson gson = new GsonBuilder().serializeNulls().create();
    String db_name = "";
    int port = 0;

    @RequestMapping(value = "/export-product-type", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelProductType(
        ...
        String url = "/export-product-type";

        try{
            mdlProductType paramResult1;
            paramResult1 = gson.fromJson(param, mdlProductType.class);
...
//            db_name = "db_ffa"; //only-testing
                ...
                urlLink = ExportProductTypeAdapter.ExportProductType(paramResult1, paramResult2, port, db_name);
    ...
    @RequestMapping(value = "/export-product-type-template", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelProductTypeTemplate(
            @RequestHeader String authorization,
            @RequestHeader String key,
            @RequestHeader String timestamp,
            @RequestHeader String signature,
        ...
        String urlLink = "";
        String url = "/export-product-type-template";
        String apiMethod = request.getMethod();
        // get all headers for logging purpose
        Map<String, String> headerList = Collections.list(request.getHeaderNames()).stream().collect(Collectors.toMap(h -> h, request::getHeader));
        String jsonHeader = gson.toJson(headerList);
        ...
        try {
            port = request.getLocalPort();
            mdlProductType paramResult1 = gson.fromJson(param, mdlProductType.class);
            ...
            if (_mdlResultFinal.status_code.equals("ERR-00-000")) {
                ...
                urlLink = ExportProductTypeAdapter.ExportProductTypeTemplate(paramResult1, paramResult2, port,  db_name);
                if (!urlLink.equals("")) {
                    _mdlErrorSchema.error_code = "ERR-00-000";
                    _mdlResultFinal = ErrorSchemaAdapter.GetErrorSchema_V2(_mdlErrorSchema, port);
                    _mdlResultFinal.value = urlLink;
