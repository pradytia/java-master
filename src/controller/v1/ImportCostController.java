...
import adapter.ImportCostAdapter;
...
public class ImportCostController {
    final static Logger logger = LogManager.getLogger(ImportCostController.class);
    Gson gson = new GsonBuilder().serializeNulls().create();
    String db_name = "";
    int port = 0 ;

    @RequestMapping(value = "/import-cost", method = RequestMethod.POST, consumes = "multipart/form-data")
    public @ResponseBody
    mdlResultFinal ImportExcelCost(
...
//                db_name = "db_ffs"; //only-testing
                if (excelfile != null && !excelfile.isEmpty()) {
                    ...
                    if (extension.equals("xlsx") || extension.equals("xls")) {
                        ...
                        mdlResponseQuery result = ImportCostAdapter.ImportExcelCost(workbook, paramResult, db_name, port);
                        if (result.Status) {
