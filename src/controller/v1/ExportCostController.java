...
import adapter.ExportCostAdapter;
...
@RestController
@RequestMapping("/v1")
public class ExportCostController {
    final static Logger logger = LogManager.getLogger(ExportCostController.class);
    Gson gson = new GsonBuilder().serializeNulls().create();
    String db_name = "";
    int port = 0;

    @RequestMapping(value = "/export-cost", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelCost(
        ...
        try {
            ...
            port =  request.getLocalPort() ;
            mdlCost paramResult1 = gson.fromJson(param, mdlCost.class);
...
//            db_name = "db_ffa"; //only-testing
                ...
                urlLink = ExportCostAdapter.ExportCost(paramResult1, paramResult2, port, db_name);
    ...
    @RequestMapping(value = "/export-cost-template", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelCostTemplate(
        ...
        String urlLink = "";
        String url = "/export-cost-template";
        String apiMethod = request.getMethod();
        // get all headers for logging purpose
        Map<String, String> headerList = Collections.list(request.getHeaderNames()).stream().collect(Collectors.toMap(h -> h, request::getHeader));
        String jsonHeader = gson.toJson(headerList);
        ...
        try {
            ...
            if (_mdlResultFinal.status_code.equals("ERR-00-000")) {
                ...
                urlLink = ExportCostAdapter.ExportCostTemplate(paramResult1, paramResult2, port,  db_name);
                if (!urlLink.equals("")) {
                    _mdlErrorSchema.error_code = "ERR-00-000";
                    _mdlResultFinal = ErrorSchemaAdapter.GetErrorSchema_V2(_mdlErrorSchema, port);
                    _mdlResultFinal.value = urlLink;
