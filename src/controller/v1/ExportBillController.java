...
public class ExportBillController {
    final static Logger logger = LogManager.getLogger(ExportBillController.class);
    static Gson gson = new GsonBuilder().serializeNulls().create();
    String db_name = "";
    int port = 0;

    @RequestMapping(value = "/export-bill-template", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelBillTemplate(
        ...
        String url = "/export-bill-template";
        ...
        try {
            port =  request.getLocalPort() ;
            mdlBill paramResult1 = gson.fromJson(param, mdlBill.class);
...
//            db_name = "db_ffa"; //only-testing
                ...
                urlLink = ExportBillAdapter.ExportBill(paramResult1, paramResult2, port, db_name);
                if (!urlLink.equals("")) {
