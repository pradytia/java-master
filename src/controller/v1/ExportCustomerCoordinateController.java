...
import adapter.ExportCustomerCoordinateAdapter;
...
@RestController
@RequestMapping("/v1")
public class ExportCustomerCoordinateController {
    final static Logger logger = LogManager.getLogger(ExportCallPlanController.class);
    Gson gson = new GsonBuilder().serializeNulls().create();
    String db_name = "";
    int port = 0;

    @RequestMapping(value = "/export-customer-coordinate", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelCustomerCoordinate(
        ...
        String urlLink = "";
        String url = "/export-customer-coordinate";
        ...
        try {
            mdlCustomerCoordinate paramResult1 = gson.fromJson(param, mdlCustomerCoordinate.class);
...
//            db_name = "db_ffa"; //only-testing
                ...
                urlLink = ExportCustomerCoordinateAdapter.ExportCustomerCoordinate(paramResult1, paramResult2, port, db_name);
                if (!urlLink.equals("")) {
    ...
    @RequestMapping(value = "/export-customer-coordinate-all", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelCustomerCoordinateAll(
            @RequestHeader String authorization,
            @RequestHeader String key,
            @RequestHeader String timestamp,
            @RequestHeader String signature,
        ...
        String urlLink = "";
        String url = "/export-customer-coordinate-all";
        String apiMethod = request.getMethod();
        // get all headers for logging purpose
        Map<String, String> headerList = Collections.list(request.getHeaderNames()).stream().collect(Collectors.toMap(h -> h, request::getHeader));
        String jsonHeader = gson.toJson(headerList);
...
//            db_name = "db_ffa"; //only-testing
                ...
                urlLink = ExportCustomerCoordinateAdapter.ExportCustomerCoordinateAll(paramResult1, paramResult2, port, db_name);
                if (!urlLink.equals("")) {
                    _mdlErrorSchema.error_code = "ERR-00-000";
                    _mdlResultFinal = ErrorSchemaAdapter.GetErrorSchema_V2(_mdlErrorSchema, port);
                    _mdlResultFinal.value = urlLink;
