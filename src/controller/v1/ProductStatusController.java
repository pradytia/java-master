...
import adapter.ProductStatusAdapter;
...
public class ProductStatusController {
    ...
    final static Logger logger = LogManager.getLogger(ProductStatusController.class);
    static Gson gson = new GsonBuilder().serializeNulls().create();
    String db_name = "";
    int port = 0;


    @RequestMapping( value = "/check-product-status", method = RequestMethod.POST)
    public @ResponseBody mdlResultFinal GetProductStatusCheck (
        ...
        String url = "/check-product-status";
        ...
        try {
            mdlProductStatus paramResult;
            paramResult = gson.fromJson(param, mdlProductStatus.class);
...
//                db_name = "db_ffa"; //only-testing
                ...
                List<mdlProductStatus> _mdlProductStatusList = ProductStatusAdapter.GetProductStatusCheck(paramResult, db_name, port);

                if(!paramResult.product_id.equals("")){
                    if(_mdlProductStatusList.size() == 0 || _mdlProductStatusList == null){
    ...
    //Upload Product Type
    @RequestMapping( value = "/upload-product-status",
            method = RequestMethod.POST)
    public @ResponseBody mdlResultFinal UploadProductStatus (
            HttpServletResponse response,
            HttpServletRequest request,
            @RequestHeader String authorization,
            @RequestHeader String key,
        ...
        String url = "/upload-product-status";
...
//                db_name = "db_ffa"; //only-testing
                ...
                boolean result = ProductStatusAdapter.UploadProductStatus(paramResult, db_name, port);
            ...
            // TODO 3 (UploadProductStatus) : Set Data exception untuk InsertLogApi
            lException = "master_core - " + ex.toString();
            //
//            return _mdlResultFinal;
        }

        //TODO 4 (UploadProductStatus) : Set Data jsonOut dan status_api untuk InsertLogApi
        String status_api = String.valueOf(response.getStatus())+" ; "+String.valueOf(response.getBufferSize())+" B";
        //

        //TODO 5 (UploadProductStatus) : Input data ke model.Log.mdlInsertLog modelInsertLogParam
        model.Log.mdlInsertLog modelInsertLogParam = new model.Log.mdlInsertLog();
        modelInsertLogParam.api_name = url;
        modelInsertLogParam.fuction_name = "UploadProductStatus";
        ...
        //TODO 6 (UploadProductStatus) : Input data yang telah diisi ke core.LogAdapter.InsertLog
    ...
    @RequestMapping( value = "/delete-product-status",
            method = RequestMethod.POST)
    public @ResponseBody mdlResultFinal DeleteProductStatus (
            HttpServletRequest request,
            HttpServletResponse response,
            @RequestHeader String authorization,
            @RequestHeader String key,
        ...
        String url = "/delete-product-status";
        String apiMethod = request.getMethod();
        // get all headers for logging purpose
        Map<String, String> headerList = Collections.list(request.getHeaderNames()).stream().collect(Collectors.toMap(h -> h, request::getHeader));
        String jsonHeader = gson.toJson(headerList);
...
//                db_name = "db_ffa"; //only-testing
                ...
                boolean result = ProductStatusAdapter.DeleteProductStatus(paramResult, db_name, port);
    ...
    @RequestMapping(value = "/get-dropdown-product-status", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal GetDropDownProductStatus(
            HttpServletRequest request,
            HttpServletResponse response,
            @RequestHeader String authorization,
            @RequestHeader String key,
        ...
        mdlResultFinal _mdlResultFinal = new mdlResultFinal();
        mdlErrorSchema _mdlErrorSchema = new mdlErrorSchema();
        String url = "/get-dropdown-product-status";
...
//            db_name = "db_ffa"; //only-testing
                ...
                List<mdlProductStatus> productTypeList;
                productTypeList = ProductStatusAdapter.GetDropDownProductStatus(db_name, port);
