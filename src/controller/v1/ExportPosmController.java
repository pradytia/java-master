...
import adapter.ExportPosmAdapter;
...
@RestController
@RequestMapping("/v1")
public class ExportPosmController {
    final static Logger logger = LogManager.getLogger(ExportPosmController.class);
    static Gson gson = new GsonBuilder().serializeNulls().create();
    String db_name = "";
    int port = 0;

    @RequestMapping(value = "/export-posm", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelPosm(
        ...
        try {
            mdlPOSMProduct paramResult1 = gson.fromJson(param, mdlPOSMProduct.class);
...
//            db_name = "db_ffa"; //only-testing
                ...
                urlLink = ExportPosmAdapter.ExportPosm(paramResult1, paramResult2, port, db_name);
    ...
    @RequestMapping(value = "/export-posm-template", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelPosmTemplate(
            @RequestHeader String authorization,
            @RequestHeader String key,
            @RequestHeader String timestamp,
            @RequestHeader String signature,
        ...
        String urlLink = "";
        String url = "/export-posm-template";
        String apiMethod = request.getMethod();
        // get all headers for logging purpose
        Map<String, String> headerList = Collections.list(request.getHeaderNames()).stream().collect(Collectors.toMap(h -> h, request::getHeader));
        String jsonHeader = gson.toJson(headerList);
        ...
        try {
            ...
            if (_mdlResultFinal.status_code.equals("ERR-00-000")) {
                ...
                urlLink = ExportPosmAdapter.ExportPOSMTemplate(paramResult1, paramResult2, port,  db_name);
                if (!urlLink.equals("")) {
                    _mdlErrorSchema.error_code = "ERR-00-000";
                    _mdlResultFinal = ErrorSchemaAdapter.GetErrorSchema_V2(_mdlErrorSchema, port);
                    _mdlResultFinal.value = urlLink;
