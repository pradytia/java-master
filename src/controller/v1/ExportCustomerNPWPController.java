...
@RestController
@RequestMapping("/v1")
public class ExportCustomerNPWPController {
    final static Logger logger = LogManager.getLogger(ExportCustomerNPWPController.class);
    Gson gson = new GsonBuilder().serializeNulls().create();
    String db_name = "";
    int port = 0;

    @RequestMapping(value = "/export-customer-NPWP", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelCustomer(
        ...
        String urlLink = "";
        String url = "/export-customer-NPWP";
        ...
        try {
            mdlCustomerNPWP paramResult1 = gson.fromJson(param, mdlCustomerNPWP.class);
...
//            db_name = "db_ffa"; //only-testing
                ...
                urlLink = adapter.CustomerNPWPAdapter.ExportCustomerNPWP(paramResult1, paramResult2, port, db_name);
                if (!urlLink.equals("")) {
    ...
    @RequestMapping(value = "/export-customer-NPWP-all", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelCustomerNPWPAll(
            @RequestHeader String authorization,
            @RequestHeader String key,
            @RequestHeader String timestamp,
            @RequestHeader String signature,
        ...
        String urlLink = "";
        String url = "/export-customer-NPWP-all";
        String apiMethod = request.getMethod();
        // get all headers for logging purpose
        Map<String, String> headerList = Collections.list(request.getHeaderNames()).stream().collect(Collectors.toMap(h -> h, request::getHeader));
        String jsonHeader = gson.toJson(headerList);
...
//            db_name = "db_ffa"; //only-testing
                ...
                urlLink = adapter.CustomerNPWPAdapter.ExportCustomerNPWPAll(paramResult1, paramResult2, port, db_name);
                if (!urlLink.equals("")) {
                    _mdlErrorSchema.error_code = "ERR-00-000";
                    _mdlResultFinal = ErrorSchemaAdapter.GetErrorSchema_V2(_mdlErrorSchema, port);
                    _mdlResultFinal.value = urlLink;
