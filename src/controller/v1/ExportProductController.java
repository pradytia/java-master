...
import adapter.ExportProductAdapter;
...
public class ExportProductController {
    final static Logger logger = LogManager.getLogger(ExportProductController.class);
    ...
    public @ResponseBody
    mdlResultFinal ExportExcelProduct(
        ...
        try {
            mdlProduct paramResult1 = gson.fromJson(param, mdlProduct.class);
...
//            db_name = "db_ffa"; //only-testing
                ...
                urlLink = ExportProductAdapter.ExportProduct(paramResult1, paramResult2, port, db_name);
    ...
    @RequestMapping(value = "/export-product-template", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelProductTemplate(
            @RequestHeader String authorization,
            @RequestHeader String key,
            @RequestHeader String timestamp,
            @RequestHeader String signature,
        ...
        String urlLink = "";
        String url = "/export-product-template";
        String apiMethod = request.getMethod();
        // get all headers for logging purpose
        Map<String, String> headerList = Collections.list(request.getHeaderNames()).stream().collect(Collectors.toMap(h -> h, request::getHeader));
        String jsonHeader = gson.toJson(headerList);
        ...
        try {
            ...
            if (_mdlResultFinal.status_code.equals("ERR-00-000")) {
                ...
                urlLink = ExportProductAdapter.ExportProductTemplate(paramResult1, paramResult2, port,  db_name);
                if (!urlLink.equals("")) {
                    _mdlErrorSchema.error_code = "ERR-00-000";
                    _mdlResultFinal = ErrorSchemaAdapter.GetErrorSchema_V2(_mdlErrorSchema, port);
                    _mdlResultFinal.value = urlLink;
