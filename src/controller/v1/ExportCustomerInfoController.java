...
@RestController
@RequestMapping("/v1")
public class ExportCustomerInfoController {
    final static Logger logger = LogManager.getLogger(ExportCustomerInfoController.class);
    Gson gson = new GsonBuilder().serializeNulls().create();
    String db_name = "";
    int port = 0;

    @RequestMapping(value = "/export-customer-info", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelCustomer(
        ...
        String urlLink = "";
        String url = "/export-customer-info";
        ...
        try {
            mdlCustomerInfo paramResult1 = gson.fromJson(param, mdlCustomerInfo.class);
...
//            db_name = "db_ffa"; //only-testing
                ...
                urlLink = adapter.CustomerInfoAdapter.ExportCustomerInfo(paramResult1, paramResult2, port, db_name);
    ...
    @RequestMapping(value = "/export-customer-info-all", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelCustomerAll(
            @RequestHeader String authorization,
            @RequestHeader String key,
            @RequestHeader String timestamp,
            @RequestHeader String signature,
        ...
        String urlLink = "";
        String url = "/export-customer-info-all";
        String apiMethod = request.getMethod();
        // get all headers for logging purpose
        Map<String, String> headerList = Collections.list(request.getHeaderNames()).stream().collect(Collectors.toMap(h -> h, request::getHeader));
        String jsonHeader = gson.toJson(headerList);
...
//            db_name = "db_ffa"; //only-testing
                ...
                urlLink = adapter.CustomerInfoAdapter.ExportCustomerInfoAll(paramResult1, paramResult2, port, db_name);
