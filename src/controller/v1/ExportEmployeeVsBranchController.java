...
import adapter.ExportEmployeeVsBranchAdapter;
...
@RestController
@RequestMapping("/v1")
public class ExportEmployeeVsBranchController {
    final static Logger logger = LogManager.getLogger(ExportEmployeeVsBranchController.class);
    static Gson gson = new GsonBuilder().serializeNulls().create();
    String db_name = "";
    int port = 0;

    @RequestMapping(value = "/export-employee-vs-branch", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelEmployeeVsBranch(
        ...
        String url = "/export-employee-vs-branch";
        ...
        try {
            mdlEmployeeVsBranch paramResult1 = gson.fromJson(param, mdlEmployeeVsBranch.class);
...
//            db_name = "db_ffa"; //only-testing
                ...
                urlLink = ExportEmployeeVsBranchAdapter.ExportEmployeeVsBranch(paramResult1, paramResult2, port, db_name);
    ...
    @RequestMapping(value = "/export-employee-vs-branch-template", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelEmplvsBchTemplate(
            @RequestHeader String authorization,
            @RequestHeader String key,
            @RequestHeader String timestamp,
            @RequestHeader String signature,
        ...
        String urlLink = "";
        String url = "/export-employee-vs-branch-template";
        String apiMethod = request.getMethod();
        // get all headers for logging purpose
        Map<String, String> headerList = Collections.list(request.getHeaderNames()).stream().collect(Collectors.toMap(h -> h, request::getHeader));
        String jsonHeader = gson.toJson(headerList);
        ...
        try {
            ...
            if (_mdlResultFinal.status_code.equals("ERR-00-000")) {
                ...
                urlLink = ExportEmployeeVsBranchAdapter.ExportEmployeeVsBranchTemplate(paramResult1, paramResult2, port,  db_name);
                if (!urlLink.equals("")) {
                    _mdlErrorSchema.error_code = "ERR-00-000";
                    _mdlResultFinal = ErrorSchemaAdapter.GetErrorSchema_V2(_mdlErrorSchema, port);
                    _mdlResultFinal.value = urlLink;
