...
import adapter.CompetitorActivityTypeAdapter;
...
@RestController
@RequestMapping("/v1")
public class CompetitorActivityTypeController {
    final static Logger logger = LogManager.getLogger(CompetitorActivityController.class);
    static Gson gson = new GsonBuilder().serializeNulls().create();
    String db_name = "";
    int port = 0;

    @RequestMapping(value = "/upload-competitor-type", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal UploadActivityTypeWeb(
        ...
        String url = "/upload-competitor-type";
        ...
        //TODO 1 (UploadActivityTypeWeb) : Param untuk InsertLogApi
        String lException = "";
        String  is_error = "";
        try {
            Type listType = new TypeToken<List<mdlCompetitorActivityType>>() {
            }.getType();
            List<mdlCompetitorActivityType> _mdlCompetitorActivityTypeList = gson.fromJson(param, listType);
...
//            db_name = "db_ffa"; //only-testing
                ...
                boolean result = CompetitorActivityTypeAdapter.UploadActivityWeb(_mdlCompetitorActivityTypeList, db_name, port);
            ...
            // TODO 3 (UploadActivityTypeWeb) : Set Data exception untuk InsertLogApi
        ...
        if(is_error.equals("1")  || mdlLogApiConfig.is_active == true) {
            //TODO 5 (UploadActivityTypeWeb) : Input data ke model.Log.mdlInsertLog modelInsertLogParam
            model.Log.mdlInsertLog modelInsertLogParam = new model.Log.mdlInsertLog();
            modelInsertLogParam.api_name = url;
            modelInsertLogParam.fuction_name = "UploadActivityTypeWeb";
            ...
            //TODO 6 (UploadActivityTypeWeb) : Input data yang telah diisi ke core.LogAdapter.InsertLog
