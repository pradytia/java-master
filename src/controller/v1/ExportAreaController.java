...
import adapter.ExportAreaAdapter;
...
@RestController
@RequestMapping("/v1")
public class ExportAreaController {
    final static Logger logger = LogManager.getLogger(ExportAreaController.class);
    static Gson gson = new GsonBuilder().serializeNulls().create();
    String db_name = "";
    int port = 0;

    @RequestMapping(value = "/export-area", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelArea(
        ...
        String urlLink = "";
        try {
            mdlArea paramResult1 = gson.fromJson(param, mdlArea.class);
...
//            db_name = "db_ffa"; //only-testing
                ...
                urlLink = ExportAreaAdapter.ExportArea(paramResult1, paramResult2, port, db_name);
                if (!urlLink.equals("")) {
    ...
    @RequestMapping(value = "/export-area-template", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelAreaTemplate(
            @RequestHeader String authorization,
            @RequestHeader String key,
            @RequestHeader String timestamp,
            @RequestHeader String signature,
        ...
        String urlLink = "";
        String url = "/export-area-template";
        String apiMethod = request.getMethod();
        // get all headers for logging purpose
        Map<String, String> headerList = Collections.list(request.getHeaderNames()).stream().collect(Collectors.toMap(h -> h, request::getHeader));
        String jsonHeader = gson.toJson(headerList);
        ...
        try {
            ...
            if (_mdlResultFinal.status_code.equals("ERR-00-000")) {
                ...
                urlLink = ExportAreaAdapter.ExportAreaTemplate(paramResult1, paramResult2, port,  db_name);
                if (!urlLink.equals("")) {
                    _mdlErrorSchema.error_code = "ERR-00-000";
                    _mdlResultFinal = ErrorSchemaAdapter.GetErrorSchema_V2(_mdlErrorSchema, port);
                    _mdlResultFinal.value = urlLink;
