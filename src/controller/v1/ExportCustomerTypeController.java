...
@RestController
@RequestMapping("/v1")
public class ExportCustomerTypeController {
    final static Logger logger = LogManager.getLogger(ExportCustomerTypeController.class);
    Gson gson = new GsonBuilder().serializeNulls().create();
    String db_name = "";
    int port = 0;

    @RequestMapping(value = "/export-customer-type", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelCustomerType(
        ...
        String urlLink = "";
        String url = "/export-customer-type";
        ...
        try {
            mdlCustomerType paramResult1 = gson.fromJson(param, mdlCustomerType.class);
...
//            db_name = "db_ffa"; //only-testing
                ...
                urlLink = ExportCustomerTypeAdapter.ExportCustomerType(paramResult1, paramResult2, port, db_name);
    ...
    @RequestMapping(value = "/export-customer-type-template", method = RequestMethod.POST)
    public @ResponseBody
    mdlResultFinal ExportExcelCustomertypeTemplate(
            @RequestHeader String authorization,
            @RequestHeader String key,
            @RequestHeader String timestamp,
            @RequestHeader String signature,
        ...
        String urlLink = "";
        String url = "/export-customer-type-template";
        String apiMethod = request.getMethod();
        // get all headers for logging purpose
        Map<String, String> headerList = Collections.list(request.getHeaderNames()).stream().collect(Collectors.toMap(h -> h, request::getHeader));
        String jsonHeader = gson.toJson(headerList);
        ...
        try {
            ...
            if (_mdlResultFinal.status_code.equals("ERR-00-000")) {
                ...
                urlLink = ExportCustomerTypeAdapter.ExportCustomerTypeTemplate(paramResult1, paramResult2, port,  db_name);
                if (!urlLink.equals("")) {
                    _mdlErrorSchema.error_code = "ERR-00-000";
                    _mdlResultFinal = ErrorSchemaAdapter.GetErrorSchema_V2(_mdlErrorSchema, port);
                    _mdlResultFinal.value = urlLink;
