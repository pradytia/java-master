...
public class ExportQuestionSetAdapter {
    final static Logger logger = LogManager.getLogger(ExportQuestionSetAdapter.class);
    static Gson gson = new GsonBuilder().serializeNulls().create();

    public static String ExportQuestionSet(mdlQuestionSet param1, mdlExportParam param2, int port, String db_name) {
        String sql = "";
        ...
        try {
            ...
            sql = "{call sp_question_set_for_export()}";
            ...
            String[] columns = {"ID Set Pertanyaan", "Isi Set Pertanyaan"};

            Sheet sheet = workbook.createSheet("Set Pertanyaan");
            ...
            int rowNum = 1;
            for (mdlQuestionSet _mdlQuestionSet : mdlQuestionSetList) {
                Row row = sheet.createRow(rowNum++);
                row.createCell(0).setCellValue(_mdlQuestionSet.question_set_id);
                row.createCell(1).setCellValue(_mdlQuestionSet.question_set_text);
            ...
            try {
                ...
                String fileName = "SetPertanyaan_" + DateHelper.GetDateTimeNowCustomFormat("yyyyMMddHHmmss");
                String url = new String(paramFileUrl + param2.app_name + "/QuestionSet/" + fileName + ".xlsx");
                File createFile = new File(paramFilePath + param2.app_name + "/QuestionSet");
                createFile.mkdirs();
    ...
    public static String ExportQuestionSetTemplate(mdlQuestionSet param1, mdlExportParam param2, int port, String db_name) {
        long startTime = System.currentTimeMillis();
        String functionName = Thread.currentThread().getStackTrace()[1].getMethodName();
        String sql = "";
        String urlLink = "";
        ...
        try {
            ...
            for (int i = 0; i < 1; i++) {
                ...
                row.createCell(1).setCellValue("Contoh (Pertanyaan A)");
            }
            ...
            try {
                ...
                String fileName = "SetPertanyaan_" + DateHelper.GetDateTimeNowCustomFormat("yyyyMMddHHmmss");
                String url = paramFileUrl + param2.app_name + "/QuestionSet/" + fileName + ".xlsx";
                File createFile = new File(paramFilePath + param2.app_name + "/QuestionSet");
                createFile.mkdirs();
