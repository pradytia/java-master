...
public class ExportStockAdapter {
    final static Logger logger = LogManager.getLogger(ExportProductPriceAdapter.class);
    static Gson gson = new GsonBuilder().serializeNulls().create();

    public static String ExportStockTemplate(mdlStock param1, mdlExportParam param2, int port, String db_name) {
        ...
        try {
            //== Create Workbook==\\
            String[] columns = {"ID Produk", "ID Cabang", "Stok", "UOM"};
            ...
            for (int i = 0; i < 1; i++) {
                ...
                row.createCell(2).setCellValue("Contoh (500)");
                row.createCell(3).setCellValue("Contoh (PCS)");
            }
            ...
            try {
                ...
                String fileName = "Stok_" + DateHelper.GetDateTimeNowCustomFormat("yyyyMMddHHmmss");
                String url = paramFileUrl + param2.app_name + "/Stock/" + fileName + ".xlsx";
                File createFile = new File(paramFilePath + param2.app_name + "/Stock");
                createFile.mkdirs();
